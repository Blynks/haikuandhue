...
import type { BackgroundView, PoemView, RevisionView } from "@/lib/types";
    ...
    <main className="space-y-8">
      ...
      {today ? (
        <section className="space-y-5">
          ...
          <div className="grid gap-5 md:grid-cols-3">{today.revisions.map((revision: RevisionView) => <ArtworkPreview key={revision.id} revision={revision} />)}</div>
          <div className="rounded-[2rem] border border-white/70 bg-white/45 p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {today.poems.flatMap((poem: PoemView) => today.backgrounds.map((background: BackgroundView) => (
